    ...
    <>
      ...
      <section className="py-32 bg-background relative" id="capabilities">
        ...
        <div className="container px-4 sm:px-6 lg:px-8 relative z-10">
          ...
          <StaggerContainer className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 mt-16 max-w-7xl mx-auto">
            {features.map(({ icon: Icon, title, description, highlighted }, idx) => (
              <StaggerItem key={title}>
                <div className={`rounded-3xl border p-10 flex flex-col h-full transition-all duration-500 hover:-translate-y-2 group ${
                  highlighted
                    ? "bg-slate-900 dark:bg-slate-800 text-white border-slate-800 dark:border-slate-700 shadow-2xl relative overflow-hidden"
                    : "bg-card border-border hover:shadow-xl hover:border-primary/20 shadow-sm"
                }`}>
                  ...
                  <div className="relative z-10 flex flex-col h-full">
                    ...
                    <h3 className={`text-xl font-poppins font-bold mb-4 ${highlighted ? "text-white" : "text-foreground"}`}>
                      {title}
                    </h3>
                    <p className={`text-sm leading-relaxed mb-6 ${highlighted ? "text-white/70" : "text-muted-foreground"}`}>
                      {description}
                    </p>
                  </div>
                </div>
        ...
        <div className="container px-4 sm:px-6 lg:px-8">
          ...
          <StaggerContainer className="grid md:grid-cols-3 gap-8 mt-16 max-w-7xl mx-auto">
            {capabilities.map(({ icon: Icon, title, desc }, idx) => (
              <StaggerItem key={title}>
                <div className="bg-background p-10 rounded-3xl border border-border shadow-sm h-full flex flex-col items-center text-center hover:shadow-xl hover:border-primary/30 transition-all duration-500 group">
                  ...
                  <h3 className="text-xl font-poppins font-bold text-foreground mb-4 leading-snug group-hover:text-primary transition-colors">{title}</h3>
      ...
      <section className="py-24 bg-primary text-primary-foreground relative overflow-hidden">
        ...
        <div className="container px-4 sm:px-6 lg:px-8 relative z-10 text-center max-w-3xl mx-auto">
          <h2 className="text-3xl md:text-5xl font-poppins font-bold tracking-tight mb-6">Experience Zero-Compromise Security</h2>
