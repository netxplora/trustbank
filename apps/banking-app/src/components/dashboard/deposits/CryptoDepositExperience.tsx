...
const CryptoDepositExperience = ({ accounts, cryptoWallets, onBack }: Props) => {
  ...
  const renderTimeline = () => (
    <div className="w-full bg-card rounded-xl border shadow-sm p-4 mb-6 overflow-x-auto">
  ...
  return (
    <div className="w-full space-y-6 pb-10">
      <div className="flex items-center gap-3 mb-6">
        <Button variant="ghost" size="icon" onClick={onBack} className="hover:bg-muted shrink-0"><ArrowLeft className="w-5 h-5" /></Button>
        <div>
          <h1 className="text-2xl font-bold font-poppins text-foreground">Digital Currency Deposit</h1>
          <p className="text-sm text-muted-foreground font-sans">Securely fund your account with supported digital assets.</p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div className="lg:col-span-2 space-y-6">
            ...
            <FadeIn className="space-y-6">
              <div className="bg-card border rounded-xl shadow-sm overflow-hidden">
                ...
                <div className="p-6 flex flex-col md:flex-row gap-6 items-center md:items-start">
                  <div className="bg-white p-3 rounded-xl border shrink-0 relative group shadow-sm">
                    <QRCodeSVG id="qr-code-svg" value={selectedWallet.wallet_address} size={150} level="H" includeMargin={true} />
                    <div className="absolute inset-0 bg-black/70 flex flex-col items-center justify-center opacity-0 group-hover:opacity-100 transition-all rounded-xl backdrop-blur-sm">
                      <Button size="sm" variant="secondary" className="text-xs font-bold" onClick={downloadQRCode}>Download QR</Button>
                    </div>
                  </div>
                    ...
                    <div>
                      ...
                      <div className="flex flex-col sm:flex-row gap-2">
                        <div className="relative flex-1">
                          <Input value={selectedWallet.wallet_address} readOnly className="font-mono text-xs sm:text-sm bg-muted/20 border-primary/20 focus-visible:ring-primary/30 h-11 pr-12 text-foreground" />
                        </div>
                        <Button variant="default" onClick={() => copyToClipboard(selectedWallet.wallet_address)} className="shrink-0 h-11 px-4 font-bold shadow-sm">
                          {copied ? <><CheckCircle2 className="w-4 h-4 mr-2" /> Copied</> : <><Copy className="w-4 h-4 mr-2" /> Copy Address</>}
                        </Button>
                      </div>
                    </div>
              ...
              {step === 1 && (
                <div className="flex justify-end pt-2">
                  <Button className="font-bold h-11 px-6 shadow-sm" onClick={handleMadeTransfer} disabled={isSubmitting}>
              ...
              {step === 2 && (
                <div className="bg-card border rounded-xl shadow-sm p-8 text-center flex flex-col items-center">
                  <div className="w-16 h-16 bg-primary/10 rounded-full flex items-center justify-center mb-5 border border-primary/20">
                    <Clock className="w-8 h-8 text-primary" />
                  </div>
                  <h2 className="text-xl font-bold font-poppins text-foreground mb-3">Awaiting Transfer</h2>
                  ...
                  <Button className="font-bold h-11 px-8 shadow-sm" onClick={() => setStep(3)}>
                    Submit Transaction Hash <ArrowRight className="w-4 h-4 ml-2" />
                  </Button>
                </div>
              )}
              ...
              {step === 3 && (
                <div className="bg-card border rounded-xl p-6 shadow-sm space-y-5">
                  <div className="flex items-center gap-3 border-b pb-4">
                    ...
                    <div>
                      <h3 className="font-bold text-lg font-poppins text-foreground">Submit Verification</h3>
                      <p className="text-xs sm:text-sm text-muted-foreground">Provide the transaction details so we can verify and credit your account.</p>
                    </div>
                  </div>

                  <div className="space-y-5 pt-2">
                    <div>
                      <Label className="text-sm font-semibold">Credit To Account</Label>
                      <select className="w-full mt-1.5 h-11 rounded-md border border-input bg-background px-3 text-sm focus:ring-2 focus:ring-primary/20 outline-none transition-all" value={accountId} onChange={(e) => setAccountId(e.target.value)}>
                        <option value="">Select a destination account...</option>
                        {accounts.map(a => (
                          <option key={a.id} value={a.id}>{a.account_type.toUpperCase()} - {a.account_number} (Bal: ${a.balance.toLocaleString()})</option>
                        ))}
                      </select>
                    </div>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                      <div>
                        <Label className="text-sm font-semibold">Deposit Amount ({selectedWallet.cryptocurrency})</Label>
                        <Input type="number" step="0.000001" placeholder="e.g. 1.5" className="mt-1.5 h-11" value={amount} onChange={(e) => setAmount(e.target.value)} />
                      </div>
                      <div>
                        <Label className="text-sm font-semibold">Transaction Hash (TxID)</Label>
                        <Input placeholder="Enter the exact transaction hash..." className="mt-1.5 font-mono text-xs h-11" value={txHash} onChange={(e) => setTxHash(e.target.value)} />
                      </div>
                    </div>
                  ...
                  <div className="flex justify-end gap-3 pt-6 border-t mt-6">
                    <Button variant="outline" className="font-bold h-11 px-6" onClick={() => setStep(1)}>Back to Wallet</Button>
                    <Button className="font-bold h-11 px-8 shadow-sm" onClick={submitDeposit} disabled={isSubmitting || !accountId || !txHash || !amount || uploading}>
                      {isSubmitting ? <RefreshCw className="w-4 h-4 mr-2 animate-spin" /> : <ShieldCheck className="w-4 h-4 mr-2" />}
                      Submit Verification
                    </Button>
                  </div>
                </div>
          ...
          {step === 4 && (
            <FadeIn>
              <div className="bg-card border rounded-xl shadow-sm p-8 text-center flex flex-col items-center">
                <div className="w-20 h-20 bg-success/10 rounded-full flex items-center justify-center mb-5 border border-success/20">
                  <CheckCircle2 className="w-10 h-10 text-success" />
                </div>
                <h2 className="text-2xl font-bold font-poppins text-foreground mb-2">Verification Submitted</h2>
        ...
        <div className="space-y-6">
          ...
          <div className="bg-card border rounded-xl overflow-hidden shadow-sm relative group">
            ...
            <div className="p-6 relative z-10">
              ...
              <ul className="text-xs text-foreground/80 space-y-2.5 mb-6 font-medium">
                <li className="flex items-center gap-2.5"><Check className="w-4 h-4 text-success shrink-0" /> Secure purchase & fast onboarding</li>
                <li className="flex items-center gap-2.5"><Check className="w-4 h-4 text-success shrink-0" /> Multiple payment methods</li>
                <li className="flex items-center gap-2.5"><Check className="w-4 h-4 text-success shrink-0" /> Global availability</li>
              </ul>
              <Button className="w-full font-bold shadow-md shadow-primary/20 h-11" onClick={() => setShowThirdPartyNotice(true)}>
