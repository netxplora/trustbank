...
import { Card, CardContent, CardDescription, CardFooter, CardHeader, CardTitle } from "@trustbank/shared-ui/components/ui/card";
...
const KYCPage = () => {
  ...
  return (
    <StaggerContainer className="space-y-8 max-w-4xl mx-auto font-sans">
      <StaggerItem>
        <div>
          <h1 className="text-2xl font-bold font-poppins text-foreground mb-1">KYC Verification</h1>
          <p className="text-sm text-muted-foreground">Complete progressive identity verification to unlock higher limits and premium banking features.</p>
        </div>
      </StaggerItem>

      {/* Status Hero */}
      <StaggerItem>
        <div className={`p-6 rounded-2xl border flex flex-col md:flex-row items-center gap-6 shadow-sm ${
          kycTier === 3 ? "bg-primary/5 border-primary/20" : "bg-card"
        }`}>
          <div className="h-20 w-20 rounded-full flex items-center justify-center shrink-0 shadow-inner bg-background border">
            <ShieldCheck className={`h-10 w-10 ${kycTier >= 1 ? "text-primary" : "text-muted-foreground"}`} />
          </div>
          <div className="flex-1 text-center md:text-left">
            ...
            <p className="text-sm text-muted-foreground mt-1">
              {kycTier === 0 && "Complete Tier 1 to unlock basic banking."}
          ...
          {isPending && (
            <div className="bg-warning/10 border border-warning/20 px-4 py-2 rounded-xl text-center shrink-0 flex items-center gap-2">
              <Clock className="w-5 h-5 text-warning" />
              <div className="text-left">
                <p className="text-xs font-bold text-warning uppercase">Under Review</p>
                <p className="text-[10px] text-warning/80 font-medium">Please wait for admin approval</p>
              </div>
            </div>
          )}
        </div>
        ...
        <StaggerItem>
          <div className="bg-card border rounded-xl p-6 shadow-sm">
            <div className="flex items-center gap-3 mb-6 border-b pb-4">
              <div className="bg-primary text-primary-foreground h-8 w-8 rounded-full flex items-center justify-center font-bold">1</div>
              <div>
                <h2 className="text-lg font-bold font-poppins">Tier 1: Basic Verification</h2>
                <p className="text-xs text-muted-foreground">Unlocks Savings account, internal transfers, and limited withdrawals.</p>
              </div>
            </div>
            
            <form onSubmit={submitTier1} className="space-y-4">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div className="space-y-1.5"><Label>Full Name</Label><Input required value={formT1.fullName} onChange={e => setFormT1(p => ({...p, fullName: e.target.value}))} /></div>
                <div className="space-y-1.5"><Label>Date of Birth</Label><Input type="date" required value={formT1.dob} onChange={e => setFormT1(p => ({...p, dob: e.target.value}))} /></div>
                <div className="space-y-1.5 md:col-span-2"><Label>Residential Address</Label><Input required value={formT1.address} onChange={e => setFormT1(p => ({...p, address: e.target.value}))} /></div>
                <div className="space-y-1.5"><Label>City</Label><Input required value={formT1.city} onChange={e => setFormT1(p => ({...p, city: e.target.value}))} /></div>
                <div className="space-y-1.5"><Label>Zip / Postal Code</Label><Input required value={formT1.zip} onChange={e => setFormT1(p => ({...p, zip: e.target.value}))} /></div>
              </div>
              <Button type="submit" disabled={loading} className="w-full md:w-auto mt-4 font-bold">
                {loading ? "Verifying..." : "Complete Tier 1 Verification"} <ChevronRight className="w-4 h-4 ml-1" />
              </Button>
            </form>
          </div>
        </StaggerItem>
      ...
      {kycTier === 2 && !isPending && (
        <StaggerItem>
          <div className="bg-card border rounded-xl p-6 shadow-sm border-primary/20 bg-primary/5">
      ...
      {kycTier === 3 && (
        <StaggerItem>
          <div className="bg-success/10 border-2 border-success/20 rounded-xl p-8 shadow-sm text-center">
