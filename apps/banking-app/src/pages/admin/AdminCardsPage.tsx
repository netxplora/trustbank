...
import { StaggerContainer, StaggerItem, SlideUp, FadeIn } from "@trustbank/shared-ui/components/Motion";
  ...
  const fetchCards = async () => {
    try {
      ...
      const enriched = cardsData.map(c => ({
        ...
        type: c.is_physical ? 'Physical Debit Card' : c.card_type === 'digital' ? 'Digital Corporate Card' : 'Virtual Debit Card',
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-bold font-poppins text-foreground mb-1">Card Provisioning & Management</h1>
          <p className="text-sm text-muted-foreground font-sans">Approve and monitor institutional card allocations</p>
        </div>
          ...
          <table className="w-full font-sans">
            <thead>
              <tr className="border-b bg-muted/20">
                ...
                <th className="text-left p-4 text-xs font-semibold font-poppins text-muted-foreground">Card Designation</th>
            ...
            <tbody>
              {loading ? (
                ...
                <tr key={c.id} className="border-b last:border-primary hover:bg-muted/10 transition-colors align-middle">
                  ...
                  <td className="p-4">
                    <span className={`text-[10px] font-bold uppercase tracking-wider px-2.5 py-1 rounded-sm border ${c.status === "Active" ? "bg-success/10 text-success border-emerald-100" : c.status === "Pending" ? "bg-warning/10 text-warning border-amber-100" : c.status === "Frozen" ? "bg-primary/10 text-primary border-primary/20" : "bg-destructive/10 text-destructive border-red-100"}`}>{c.status}</span>
                  </td>
                  ...
                  <td className="p-4 text-center">
                    <div className="flex items-center justify-center gap-1.5">
                      ...
                      <Button variant="ghost" size="icon" className="h-8 w-8 hover:bg-blue-500/10 transition-colors" onClick={() => setEditingCard(c.raw)}>
            ...
            <form onSubmit={updateCard} className="space-y-4 mt-2">
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <select className="w-full rounded-lg border bg-background px-3 py-2 text-sm font-bold" value={editingCard.status || "inactive"} onChange={e => setEditingCard({...editingCard, status: e.target.value})}>
                ...
                <div>
                  ...
                  <select className="w-full rounded-lg border bg-background px-3 py-2 text-sm font-bold" value={editingCard.is_frozen ? "yes" : "no"} onChange={e => setEditingCard({...editingCard, is_frozen: e.target.value === "yes"})}>
      ...
      <Dialog open={showCreate} onOpenChange={setShowCreate}>
        <DialogContent className="max-w-lg font-sans">
          <DialogHeader>
            <DialogTitle className="font-poppins font-bold">Provision New Card for User</DialogTitle>
          ...
          <form onSubmit={handleCreateCard} className="space-y-4 mt-2">
            ...
            <div className="grid grid-cols-2 gap-4">
              <div>
                ...
                <select className="w-full rounded-lg border bg-background px-3 py-2 text-sm font-bold" value={createForm.cardType} onChange={e => setCreateForm(f => ({ ...f, cardType: e.target.value as any }))}>
              ...
              <div>
                <Label className="text-xs font-bold uppercase tracking-wider text-muted-foreground block mb-1">Status</Label>
                <select className="w-full rounded-lg border bg-background px-3 py-2 text-sm font-bold" value={createForm.status} onChange={e => setCreateForm(f => ({ ...f, status: e.target.value }))}>
                  <option value="active">Active</option>
                  <option value="inactive">Inactive</option>
                </select>
              </div>
