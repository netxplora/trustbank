...
const AdminLoansPage = () => {
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-2xl font-bold font-poppins text-foreground mb-1">Institutional Credit Facilities</h1>
        <p className="text-sm text-muted-foreground font-sans">Review, underwrite, and manage lending portfolios</p>
      </div>
      ...
      <StaggerContainer className="grid grid-cols-2 md:grid-cols-4 gap-4 font-sans">
        {[
          ...
          { label: "Active Facilities", value: activeFacilities.toString(), color: "text-success" },
          { label: "Pending Underwriting", value: pendingUnderwriting.toString(), color: "text-warning" },
          ...
          <table className="w-full font-sans">
            <thead>
              <tr className="border-b bg-muted/20">
                <th className="text-left p-4 text-xs font-semibold font-poppins text-muted-foreground">Borrower</th>
                <th className="text-left p-4 text-xs font-semibold font-poppins text-muted-foreground">Facility Purpose</th>
                <th className="text-right p-4 text-xs font-semibold font-poppins text-muted-foreground">Principal Amount</th>
                <th className="text-left p-4 text-xs font-semibold font-poppins text-muted-foreground hidden md:table-cell">Tenure</th>
                <th className="text-left p-4 text-xs font-semibold font-poppins text-muted-foreground">Status</th>
                <th className="text-right p-4 text-xs font-semibold font-poppins text-muted-foreground hidden lg:table-cell">Outstanding</th>
            ...
            <tbody>
              {filtered.length === 0 ? (
                ...
                <tr key={l.id} className="border-b last:border-primary hover:bg-muted/10 transition-colors">
                  ...
                  <td className="p-4"><span className="text-[10px] font-bold uppercase tracking-wider bg-muted/50 border px-2.5 py-1 rounded-sm max-w-[150px] truncate block">{l.purpose || "General Credit"}</span></td>
                  ...
                  <td className="p-4">
                    <span className={`text-[10px] font-bold uppercase tracking-wider px-2.5 py-1 rounded-sm border ${l.status === "approved" || l.status === "active" ? "bg-success/10 text-success border-emerald-100" : l.status === "pending" ? "bg-warning/10 text-warning border-amber-100" : "bg-destructive/10 text-destructive border-red-100"}`}>{l.status}</span>
                  </td>
      ...
      <Dialog open={!!manageLoan} onOpenChange={(open) => !open && setManageLoan(null)}>
        <DialogContent className="font-sans max-w-lg">
          <DialogHeader>
            <DialogTitle className="font-poppins text-xl">Manage Credit Facility</DialogTitle>
            <DialogDescription>
              Review or modify the terms of this application before making an underwriting decision.
            </DialogDescription>
          </DialogHeader>

          {manageLoan && (
            <div className="space-y-5 my-2">
              <div className="grid grid-cols-2 gap-4 bg-muted/10 p-4 rounded-lg border">
                <div>
                ...
                <div>
                  ...
                  <p className="text-sm font-semibold text-foreground truncate">{manageLoan.purpose || "General Credit"}</p>
                </div>
              </div>
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <Input type="number" disabled={manageLoan.status !== "pending"} value={editForm.amount} onChange={e => setEditForm(f => ({ ...f, amount: e.target.value }))} className="font-mono font-bold" />
                </div>
                <div>
                  <label className="text-xs font-bold uppercase tracking-wider text-muted-foreground mb-1.5 block">Term (Months)</label>
                  <select disabled={manageLoan.status !== "pending"} className="w-full rounded-lg border bg-background px-3 py-2 text-sm font-semibold h-10" value={editForm.tenure_months} onChange={e => setEditForm(f => ({ ...f, tenure_months: e.target.value }))}>
                ...
                <div>
                  ...
                  <Input type="number" step="0.01" disabled={manageLoan.status !== "pending"} value={editForm.interest_rate} onChange={e => setEditForm(f => ({ ...f, interest_rate: e.target.value }))} className="font-mono font-bold" />
                </div>
                <div>
                  <label className="text-xs font-bold uppercase tracking-wider text-muted-foreground mb-1.5 block">Est. Monthly Payment</label>
                  <div className="h-10 flex items-center px-3 bg-muted/20 border rounded-lg">
