            ...
            <div>
              <h1 className="text-lg font-bold font-poppins text-foreground flex items-center gap-2">
                ...
                {editingPage.is_published ? (
                  <span className="text-[10px] font-bold uppercase tracking-wider px-2 py-0.5 bg-success/10 text-success border border-emerald-500/20 rounded-full">Published</span>
                ) : (
                  <span className="text-[10px] font-bold uppercase tracking-wider px-2 py-0.5 bg-warning/10 text-yellow-600 border border-warning/20 rounded-full">Draft</span>
                )}
              </h1>
        ...
        <div className={`grid gap-6 ${showPreview ? "xl:grid-cols-2" : "lg:grid-cols-3"}`}>
          ...
          <div className={`${showPreview ? "" : "lg:col-span-2"} space-y-4`}>
            ...
            <div className="relative">
              ...
              {showPalette && (
                <div className="absolute z-20 top-14 left-0 right-0 bg-card border rounded-xl shadow-lg p-3 grid grid-cols-2 md:grid-cols-4 gap-2 animate-in fade-in slide-in-from-top-2 duration-200">
            ...
            {editingPage.content_blocks.length === 0 && (
              <div className="text-center py-16 text-muted-foreground bg-muted/10 rounded-xl border-dashed border-2">
          ...
          {showPreview ? (
            <div className="space-y-4">
              <div className="bg-card border rounded-xl overflow-hidden shadow-sm sticky top-4">
            ...
            <div className="space-y-4">
              ...
              <div className="bg-card border rounded-xl p-5 space-y-4 shadow-sm">
                ...
                <div>
                  ...
                  <Input value={editingPage.title} onChange={e => updatePage(p => ({ ...p, title: e.target.value }))} className="font-semibold" />
                </div>
                <div>
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
        <div>
          <h1 className="text-2xl font-bold font-poppins text-foreground mb-1">Page Builder</h1>
          <p className="text-sm text-muted-foreground font-sans">Create and manage dynamic content pages with the visual block editor.</p>
        </div>
        <Button onClick={handleCreateNew} className="font-bold shrink-0">
      ...
      <div className="relative max-w-md">
        ...
        <Input placeholder="Search pages..." value={searchQuery} onChange={e => setSearchQuery(e.target.value)} className="pl-9 bg-card" />
      </div>
            ...
            {loading ? (
              ...
              filteredPages.map(page => (
                <tr key={page.id} className="hover:bg-muted/10 transition-colors">
                  ...
                  <td className="px-6 py-4">
                    <span className={`px-2.5 py-1 text-[10px] font-bold uppercase rounded-full border ${page.is_published ? "bg-success/10 text-success border-emerald-500/20" : "bg-warning/10 text-yellow-600 border-warning/20"}`}>
          ...
          {block.type === "testimonials" && (
            <ArrayBlockEditor
              ...
              fields={[
                ...
                { key: "role", label: "Role / Location", placeholder: "CEO, Lagos" },
