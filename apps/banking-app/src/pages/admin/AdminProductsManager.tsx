...
import { Switch } from "@trustbank/shared-ui/components/ui/switch";
import { StaggerContainer, StaggerItem, FadeIn, SlideUp } from "@trustbank/shared-ui/components/Motion";
import { CardSkeleton } from "@trustbank/shared-ui/components/skeletons/DashboardSkeleton";
...
export default function AdminProductsManager() {
      ...
      <SlideUp className="space-y-6 max-w-3xl">
        ...
        <div className="bg-card border rounded-xl p-6 space-y-5">
          ...
          <div className="grid grid-cols-2 gap-4">
            <div><label className="text-xs font-semibold mb-1 block">Product Name</label><Input value={editingProduct.name} onChange={e => setEditingProduct({...editingProduct, name: e.target.value})} /></div>
            <div>
              <label className="text-xs font-semibold mb-1 block">Category</label>
              <select className="flex h-10 w-full rounded-md border border-input bg-background px-3 py-2 text-sm ring-offset-background" value={editingProduct.category} onChange={e => setEditingProduct({...editingProduct, category: e.target.value})}>
          ...
          <div className="grid grid-cols-2 gap-4">
            <div><label className="text-xs font-semibold mb-1 block">Interest Rate / APY (%)</label><Input type="number" step="0.01" value={editingProduct.interest_rate || ""} onChange={e => setEditingProduct({...editingProduct, interest_rate: parseFloat(e.target.value) || null})} /></div>
            <div><label className="text-xs font-semibold mb-1 block">Monthly/Annual Fee ($)</label><Input type="number" step="0.01" value={editingProduct.fee || ""} onChange={e => setEditingProduct({...editingProduct, fee: parseFloat(e.target.value) || null})} /></div>
          </div>

          <div><label className="text-xs font-semibold mb-1 block">Image/Icon URL</label><Input value={editingProduct.image_url || ""} onChange={e => setEditingProduct({...editingProduct, image_url: e.target.value})} className="font-mono" placeholder="/assets/products/card.png" /></div>
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-bold font-poppins text-foreground mb-1">Banking Products</h1>
          <p className="text-sm text-muted-foreground font-sans">Manage checking, savings, loans, and credit card offerings.</p>
        </div>
        <Button onClick={handleCreateNew} className="font-bold"><Plus className="h-4 w-4 mr-2" /> Add Product</Button>
      </div>
      ...
      <StaggerContainer className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {loading ? (
          ...
          <div className="col-span-full py-12 text-center border-2 border-dashed rounded-xl bg-muted/10 text-muted-foreground">No products available in the catalog.</div>
        ) : (
          products.map(product => (
            <StaggerItem key={product.id}>
            <div className={`bg-card border rounded-xl p-5 relative h-full ${!product.is_active && 'opacity-60 grayscale'}`}>
