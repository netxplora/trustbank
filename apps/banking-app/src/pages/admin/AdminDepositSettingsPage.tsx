...
const AdminDepositSettingsPage = () => {
  ...
  const statusBadge = (status: string) => {
    const map: Record<string, string> = {
      pending: "bg-warning/10 text-warning border-amber-100",
      confirmed: "bg-success/10 text-success border-emerald-100",
      rejected: "bg-destructive/10 text-destructive border-red-100",
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-2xl font-bold font-poppins text-foreground mb-1">Wallet & Provider Configuration</h1>
        <p className="text-sm text-muted-foreground font-sans">Manage deposit wallets and configure third-party digital currency providers.</p>
      </div>

      <Tabs defaultValue="fiat" className="w-full">
        <TabsList className="mb-4 bg-muted/50 border">
          <TabsTrigger value="fiat" className="font-semibold text-xs">Fiat Banks</TabsTrigger>
          <TabsTrigger value="wallets" className="font-semibold text-xs">Crypto Wallets</TabsTrigger>
          <TabsTrigger value="settings" className="font-semibold text-xs">Provider Settings</TabsTrigger>
        </TabsList>
            ...
            <StaggerContainer className="grid grid-cols-1 md:grid-cols-2 gap-4 font-sans">
              {fiatBanks.map((b) => (
                <StaggerItem key={b.id}>
                  <div className={`bg-card rounded-xl border p-5 shadow-sm hover-lift h-full ${!b.is_active ? "opacity-60" : ""}`}>
                    ...
                    <div className="flex gap-2">
                      ...
                      <Button variant="outline" size="sm" className="text-xs font-semibold text-destructive border-red-100 hover:bg-destructive/10" onClick={() => deleteFiatBank(b.id)}>
            ...
            <StaggerContainer className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4 font-sans">
              {wallets.map((w) => (
                <StaggerItem key={w.id}>
                  <div className={`bg-card rounded-xl border p-5 shadow-sm hover-lift h-full ${!w.enabled ? "opacity-60" : ""}`}>
                    <div className="flex items-center justify-between mb-3">
                      <div className="flex items-center gap-2">
                        <div className="h-9 w-9 bg-warning/10 rounded-lg border border-amber-100 flex items-center justify-center overflow-hidden shrink-0">
                    ...
                    <div className="flex gap-2">
                      ...
                      <Button variant="outline" size="sm" className="text-xs font-semibold text-destructive border-red-100 hover:bg-destructive/10" onClick={() => deleteWallet(w.id)}>
                        <Trash2 className="h-3.5 w-3.5" />
                      </Button>
                    </div>
                  </div>
        ...
        <TabsContent value="settings" className="m-0">
          <FadeIn className="bg-card rounded-xl border p-6 shadow-sm max-w-2xl">
