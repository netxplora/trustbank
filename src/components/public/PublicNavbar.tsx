              ...
              <div className="relative" onMouseLeave={() => setProfileMenuOpen(false)}>
                ...
                {profileMenuOpen && (
                  <div className="absolute top-[calc(100%+4px)] right-0 min-w-[200px] bg-surface border border-border rounded-lg shadow-xl py-2 z-50 animate-in fade-in slide-in-from-top-1 duration-200">
                    ...
                    <Link to="/dashboard/profile" className="flex items-center px-4 py-2 text-xs font-sans text-muted-foreground hover:text-foreground hover:bg-muted"><UserIcon className="h-3.5 w-3.5 mr-2" /> My Profile</Link>
                    <Link to="/dashboard/accounts" className="flex items-center px-4 py-2 text-xs font-sans text-muted-foreground hover:text-foreground hover:bg-muted"><CreditCard className="h-3.5 w-3.5 mr-2" /> Accounts</Link>
                    <Link to="/dashboard/statements" className="flex items-center px-4 py-2 text-xs font-sans text-muted-foreground hover:text-foreground hover:bg-muted"><LayoutDashboard className="h-3.5 w-3.5 mr-2" /> Transactions</Link>
                    <Link to="/dashboard/security" className="flex items-center px-4 py-2 text-xs font-sans text-muted-foreground hover:text-foreground hover:bg-muted"><Settings className="h-3.5 w-3.5 mr-2" /> Settings</Link>
                    <div className="h-px bg-muted my-1" />
                    <button onClick={handleLogout} className="w-full flex items-center px-4 py-2 text-xs font-sans text-destructive hover:text-destructive/80 hover:bg-muted"><LogOut className="h-3.5 w-3.5 mr-2" /> Logout</button>
                  </div>
                )}
              </div>
              <ThemeToggle />
              ...
              <div className="flex flex-col gap-3">
                ...
                {user ? (
                  <div className="flex flex-col gap-2">
                    ...
                    <Link to="/dashboard/profile" onClick={() => setMenuOpen(false)} className="flex items-center gap-3 p-3.5 rounded-xl bg-card border border-border hover:border-primary/50 transition-all text-sm font-semibold text-foreground"><UserIcon className="h-4 w-4 text-primary" /> My Profile</Link>
                    <Link to="/dashboard/notifications" onClick={() => setMenuOpen(false)} className="flex items-center gap-3 p-3.5 rounded-xl bg-card border border-border hover:border-primary/50 transition-all text-sm font-semibold text-foreground"><Bell className="h-4 w-4 text-primary" /> Notifications</Link>
                    <Link to="/" onClick={() => setMenuOpen(false)} className="flex items-center gap-3 p-3.5 rounded-xl bg-card border border-border hover:border-primary/50 transition-all text-sm font-semibold text-foreground"><svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="text-primary"><path d="m3 9 9-7 9 7v11a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2z"/><polyline points="9 22 9 12 15 12 15 22"/></svg> Go to Homepage</Link>
                    <button onClick={() => { setMenuOpen(false); handleLogout(); }} className="flex items-center gap-3 p-3.5 rounded-xl bg-destructive/10 border border-destructive/20 hover:bg-destructive/20 transition-all text-sm font-semibold text-destructive mt-2"><LogOut className="h-4 w-4" /> Sign Out</button>
                  </div>
                ) : (
                  <div className="flex flex-col gap-3">
