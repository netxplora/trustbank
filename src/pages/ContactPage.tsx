...
const ContactPage = () => {
  ...
  return (
    <>
      ...
      <section className="py-10 sm:py-16 lg:py-24 bg-background" id="directory">
                ...
                <div className="bg-card rounded-2xl border border-muted/50 p-4 sm:p-6 shadow-sm hover-lift transition-all h-full flex flex-col justify-between">
                  <div>
                    <div className="h-10 w-10 sm:h-12 sm:w-12 rounded-xl bg-primary/10 flex items-center justify-center mb-4 sm:mb-6">
                      <dept.icon className="h-5 w-5 sm:h-6 sm:w-6 text-primary" />
                    </div>
                    <h3 className="font-poppins font-bold text-foreground mb-2 text-xs sm:text-sm">{dept.title}</h3>
                    <p className="text-[11px] sm:text-xs text-muted-foreground leading-relaxed font-sans mb-4 sm:mb-6">{dept.desc}</p>
                  </div>
                  <div className="space-y-1.5 border-t border-muted/50 pt-3 sm:pt-4">
                    <p className="text-[11px] sm:text-xs font-bold text-foreground flex items-center gap-2">
                      <Phone className="h-3.5 w-3.5 text-muted-foreground shrink-0" /> {dept.phone}
                    </p>
                    <p className="text-[11px] sm:text-xs text-muted-foreground flex items-center gap-2 truncate">
                      <Mail className="h-3.5 w-3.5 shrink-0" /> {dept.email}
                    </p>
                  </div>
                </div>
              </StaggerItem>
      ...
      <section className="py-10 sm:py-16 lg:py-24 bg-surface border-y border-muted/50" id="consultation">
        <div className="container px-4 sm:px-6 lg:px-8">
          <div className="grid grid-cols-1 md:grid-cols-5 gap-8 sm:gap-16 items-start">
            <SlideUp className="md:col-span-3">
              <div className="bg-card rounded-2xl p-4 sm:p-8 border border-muted/50 shadow-elevated">
                ...
                <form onSubmit={handleSubmit} className="space-y-6">
                  <div className="grid grid-cols-2 gap-6">
                    <div className="space-y-2">
                      ...
                      <Input name="firstName" value={formData.firstName} onChange={handleChange} required className="bg-background border-muted h-11 text-base" />
                    </div>
                    <div className="space-y-2">
                      <label className="text-xs font-bold text-foreground uppercase tracking-wider">Last Name</label>
                      <Input name="lastName" value={formData.lastName} onChange={handleChange} required className="bg-background border-muted h-11 text-base" />
                    </div>
                  </div>
                  ...
                  <div className="grid grid-cols-2 gap-6">
                    <div className="space-y-2">
                      ...
                      <Input type="email" name="email" value={formData.email} onChange={handleChange} required className="bg-background border-muted h-11 text-base" />
                    </div>
                    <div className="space-y-2">
                      <label className="text-xs font-bold text-foreground uppercase tracking-wider">Routing Department</label>
                      <select name="department" value={formData.department} onChange={handleChange} required className="flex h-11 w-full rounded-md border border-input bg-background px-3 py-2 text-base ring-offset-background file:border-primary file:bg-transparent file:text-sm file:font-medium placeholder:text-muted-foreground focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 disabled:cursor-not-allowed disabled:opacity-50 border-muted">
