  ...
  return (
    <div className="space-y-4 max-w-6xl mx-auto px-1 sm:px-4 py-2 font-sans text-xs">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-2">
        <div>
          <h1 className="text-lg sm:text-xl font-bold font-poppins text-foreground mb-0.5">Corporate Insights & Content</h1>
          <p className="text-xs text-muted-foreground font-sans">Manage articles, drafts, and schedule institutional updates</p>
        </div>
        <Button size="sm" onClick={handleOpenNew} className="shrink-0 font-bold text-xs h-8 rounded-lg">
          <Plus className="h-3.5 w-3.5 mr-1" /> Publish New Insight
        </Button>
      </div>

      <div className="flex flex-col sm:flex-row gap-2 font-sans justify-between">
        <div className="flex gap-1.5">
          {(["all", "published", "draft"] as const).map(f => (
            <button
              key={f}
              onClick={() => setFilterStatus(f)}
              className={`px-2.5 py-1 text-xs font-bold rounded-lg border transition-colors capitalize ${filterStatus === f ? "bg-primary text-primary-foreground border-primary" : "bg-card hover:bg-muted border-border/60"}`}
            >
              {f}
            </button>
          ))}
        ...
        <div className="relative w-full sm:max-w-xs">
          ...
          <Input placeholder="Search publications..." value={search} onChange={(e) => setSearch(e.target.value)} className="pl-8 bg-card h-8 text-xs rounded-lg" />
        </div>
      </div>
                  ...
                  return (
                    <tr key={post.id} className="hover:bg-muted/10 transition-colors">
                      ...
                      <td className="p-4 align-top space-y-2">
                        <span className={`text-[10px] border font-bold px-2.5 py-1 rounded-sm uppercase tracking-wider block w-max ${isDraft ? "bg-warning/10 text-warning border-warning/20" : isScheduled ? "bg-primary/10 text-primary border-primary/20" : "bg-success/10 text-success border-success/20"}`}>
            ...
            <div className="grid grid-cols-3 gap-6">
              <div className="col-span-2 space-y-4">
                <div>
                  ...
                  <Input id="title" value={title} onChange={(e) => setTitle(e.target.value)} required className="mt-1 font-semibold h-11 text-base" />
                </div>
              ...
              <div className="space-y-4">
                <div className="bg-muted/30 p-3 rounded-lg border">
                  ...
                  <select value={status} onChange={(e) => setStatus(e.target.value as "published" | "draft")} className="w-full rounded-md border mt-1.5 px-3 py-2 h-11 text-base font-semibold bg-background">
                ...
                <div>
                  ...
                  <Input type="datetime-local" value={publishedAt} onChange={(e) => setPublishedAt(e.target.value)} className="mt-1 text-base h-11" />
                </div>

                <div>
                  <Label className="text-xs font-semibold text-muted-foreground">Sector Category</Label>
                  <select value={category} onChange={(e) => setCategory(e.target.value)} className="w-full rounded-md border mt-1 px-3 py-2 h-11 text-base bg-background">
                ...
                <div className="border-t pt-4">
                  ...
                  {imageOption === "url" ? (
                    <Input placeholder="https://..." value={imageUrl} onChange={(e) => setImageUrl(e.target.value)} className="h-11 text-base" />
                  ) : (
