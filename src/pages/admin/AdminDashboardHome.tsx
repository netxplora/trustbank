...
// Lazy-load Recharts — 272KB chunk only loaded when charts become visible
const LazyCharts = lazy(() => import("@/components/dashboard/AdminCharts"));

const ChartSkeleton = () => (
  <div className="w-full h-48 bg-muted/20 animate-pulse rounded-xl" />
);
...
const AdminDashboardHome = () => {
  ...
  const fetchStats = async () => {
    ...
    const { data: recentTx } = await (supabase as any)
      .from("transactions")
  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-3 gap-4 max-w-6xl mx-auto px-1 sm:px-4 py-2 font-sans">
      ...
      {/* Lazy-loaded charts — recharts bundle deferred until needed */}
      <Suspense fallback={<><ChartSkeleton /><ChartSkeleton /></>}>
        <LazyCharts
          loanDistribution={loanDistribution}
          txVolumeData={txVolumeData}
        />
      </Suspense>
