...
const AdminNotificationsPage = () => {
  ...
  return (
    <div className="space-y-4 max-w-6xl mx-auto px-1 sm:px-4 py-2 font-sans">
      <div>
        <h1 className="text-lg sm:text-xl font-bold font-poppins text-foreground mb-0.5">Global Broadcasts & Notifications</h1>
        <p className="text-xs text-muted-foreground font-sans">Dispatch critical announcements to institutional portfolios</p>
      </div>

      <StaggerContainer className="grid lg:grid-cols-5 gap-4 font-sans text-xs">
        <StaggerItem className="lg:col-span-3">
          <div className="bg-card rounded-xl border border-border/60 p-3.5 sm:p-4 shadow-sm h-full">
            <h2 className="font-bold font-poppins text-foreground text-xs mb-3 flex items-center gap-1.5 border-b border-border/60 pb-2.5"><Send className="h-4 w-4 text-primary" /> Dispatch Broadcast</h2>
            <form onSubmit={handleSend} className="space-y-5">
              <div>
                ...
                <Input placeholder="Enter official subject..." className="font-semibold h-11 text-base" required value={subject} onChange={e => setSubject(e.target.value)} />
              </div>
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <select className="w-full rounded-md border bg-background px-3 py-2.5 h-11 text-base font-semibold" value={channel} onChange={e => setChannel(e.target.value)}>
                ...
                <div>
                  ...
                  <select className="w-full rounded-md border bg-background px-3 py-2.5 h-11 text-base font-semibold" value={audience} onChange={e => setAudience(e.target.value)}>
        ...
        <StaggerItem className="lg:col-span-2">
          <div className="bg-card rounded-3xl border shadow-sm hover-lift h-full">
