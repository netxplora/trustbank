  ...
  return (
    <div className="space-y-4 max-w-6xl mx-auto px-1 sm:px-4 py-2 font-sans">
      <div>
        ...
        <p className="text-xs text-muted-foreground font-sans">Review customer identity verification requests</p>
      </div>
        ...
        {[
          ...
          <StaggerItem key={s.label}>
            <div className="bg-card rounded-xl border border-border/60 p-3 shadow-sm h-full">
              ...
              <p className={`text-base font-bold mt-0.5 ${s.color || "text-foreground"}`}>{s.value}</p>
            </div>
          </StaggerItem>
        ))}
      </StaggerContainer>
      ...
      <div className="flex flex-col sm:flex-row gap-2 font-sans">
        <div className="relative flex-1">
          ...
          <Input placeholder="Search applicants by name or ID..." value={search} onChange={(e) => setSearch(e.target.value)} className="pl-8 h-8 text-xs rounded-lg" />
        </div>
          ...
          <table className="w-full font-sans text-xs">
            <thead>
              <tr className="border-b border-border/60 bg-muted/10">
                <th className="text-left px-3 py-2.5 text-[11px] font-semibold font-poppins text-muted-foreground">Customer Profile</th>
                ...
                <th className="text-left px-3 py-2.5 text-[11px] font-semibold font-poppins text-muted-foreground">Verification Status</th>
                <th className="text-left px-3 py-2.5 text-[11px] font-semibold font-poppins text-muted-foreground hidden lg:table-cell">Submission Date</th>
                <th className="text-center px-3 py-2.5 text-[11px] font-semibold font-poppins text-muted-foreground">Audit Actions</th>
              </tr>
            </thead>
            <tbody>
              {loading ? (
      ...
      <Dialog open={!!selectedKYC} onOpenChange={() => setSelectedKYC(null)}>
        <DialogContent className="max-w-lg font-sans">
          <DialogHeader>
            <DialogTitle className="font-poppins">Verification Profile — {selectedKYC?.name}</DialogTitle>
          ...
          {selectedKYC && (
            <div className="space-y-4 mt-2">
              <div className="grid grid-cols-2 gap-3 text-sm">
                ...
                <div className="bg-muted/30 border rounded-xl p-3 col-span-2 flex justify-between items-center">
