...
import { Send, Search, User, MessageCircle, CheckCircle, XCircle, Sparkles, Check, CheckCheck, Loader2, Circle, ArrowLeft, ShieldAlert, UserCheck, Paperclip, FileText, Download } from "lucide-react";
...
const statusConfig: Record<string, { bg: string; dot: string; label: string }> = {
  open: { bg: "bg-success/10 text-success border-success/20", dot: "bg-success", label: "Open" },
  pending: { bg: "bg-warning/10 text-warning border-warning/20", dot: "bg-warning", label: "Pending" },
...
const AdminChatPage = () => {
  ...
  const handleFileChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    if (e.target.files && e.target.files[0]) {
      ...
      if (!allowedTypes.includes(file.type)) {
        toast.error("Only JPG, PNG, WEBP, or PDF documents are allowed.");
        return;
      }
  ...
  const uploadAttachment = async (file: File) => {
    ...
    const fileName = `admin_${user.id}/${Math.random().toString(36).substring(2, 15)}.${fileExt}`;
  ...
  const sendMessage = async () => {
    if ((!input.trim() && !attachment) || !selectedConv || !user) return;
  ...
  return (
    <div className="space-y-4 max-w-6xl mx-auto px-1 sm:px-4 py-2 font-sans text-xs">
      <div>
        <h1 className="text-lg sm:text-xl font-bold font-poppins text-foreground mb-0.5">Corporate Client Support</h1>
        <p className="text-xs text-muted-foreground">Manage client inquiries and secure communications</p>
      </div>

      <div className="bg-card rounded-xl border border-border/60 shadow-sm flex overflow-hidden" style={{ height: "calc(100vh - 180px)" }}>
        {/* ─── Left: Inbox Sidebar ─── */}
        <div className={`w-full md:w-80 border-r border-border/60 flex-col shrink-0 ${selectedConv ? "hidden md:flex" : "flex"}`}>
                    ...
                    >
                      <div className="flex items-start gap-3">
                        ...
                        <div className="relative shrink-0">
                          ...
                          {conv.status === "open" && (
                            <span className="absolute -bottom-0.5 -right-0.5 h-3.5 w-3.5 bg-success border-2 border-card rounded-full" />
                          )}
                        </div>
          ...
          {selectedConv ? (
            <FadeIn className="flex-1 flex flex-col min-w-0 overflow-hidden">
                ...
                <div className="flex gap-2">
                  {selectedConv.status !== "resolved" && (
                    <Button variant="outline" size="sm" className="text-xs font-bold h-8 rounded-lg" onClick={() => updateStatus("resolved")}>
                      <CheckCircle className="h-3.5 w-3.5 mr-1.5 text-success" /> Resolve
                    </Button>
                  )}
                          ...
                          <div className="mb-2">
                            {msg.file_url.match(/\.(jpeg|jpg|gif|png|webp)$/i) ? (
                              <a href={msg.file_url} target="_blank" rel="noopener noreferrer">
                                <img src={msg.file_url} alt="attachment" className="rounded-lg max-h-48 object-cover border" />
                              </a>
                            ) : (
                              <a href={msg.file_url} target="_blank" rel="noopener noreferrer" className={`flex items-center gap-2 p-3 rounded-lg border ${isAdmin ? 'bg-primary-foreground/10 border-primary-foreground/20' : 'bg-muted border-border'} hover:opacity-80 transition-opacity`}>
                                <FileText className="h-5 w-5" />
                                <span className="text-xs font-semibold truncate max-w-[150px]">Document</span>
              ...
              <div className="p-4 border-t bg-background">
                ...
                {attachment && (
                  <div className="mb-3 flex items-center gap-2 bg-muted/50 p-2 rounded-lg border text-sm">
                    <FileText className="h-4 w-4 text-primary" />
                    <span className="flex-1 truncate font-medium">{attachment.name}</span>
                    <button onClick={() => setAttachment(null)} className="text-muted-foreground hover:text-destructive p-1 rounded-full hover:bg-background">
                ...
                <div className="flex items-end gap-2 bg-muted/30 p-1.5 rounded-2xl border focus-within:ring-2 focus-within:ring-primary/20 transition-all">
                  ...
                  <Button
                    ...
                    size="icon"
                    variant="ghost"
                    className="h-11 w-11 shrink-0 rounded-xl hover:bg-muted text-muted-foreground hover:text-foreground"
                  ...
                  <Input
                    ...
                    className="flex-1 border-0 bg-transparent shadow-none focus-visible:ring-0 text-sm h-11 px-1"
                  />
                  <Button size="icon" onClick={sendMessage} disabled={(!input.trim() && !attachment) || sending} className="h-11 w-11 shrink-0 rounded-2xl shadow-sm">
          ...
          <div className="w-72 border-l hidden lg:flex flex-col overflow-y-auto custom-scrollbar bg-background">
            ...
            <div className="p-5 space-y-4 text-sm flex-1">
              ...
              <div className="bg-muted/30 rounded-2xl p-4">
                ...
                <span className={`text-[10px] font-bold uppercase tracking-wider px-2.5 py-1 rounded-full border ${selectedConv.profiles?.kyc_status === "approved" ? "bg-success/10 text-success border-success/20" : "bg-warning/10 text-warning border-warning/20"}`}>
              ...
              <div className="bg-muted/30 rounded-2xl p-4">
                ...
                <span className={`text-[10px] font-bold uppercase tracking-wider px-2.5 py-1 rounded-full border ${selectedConv.profiles?.account_status === "active" ? "bg-success/10 text-success border-success/20" : "bg-destructive/10 text-destructive border-destructive/20"}`}>
            ...
            <div className="p-5 border-t bg-muted/10 space-y-3 shrink-0">
              ...
              <div className="grid grid-cols-2 gap-2">
                {selectedConv.profiles?.kyc_status !== "approved" && (
                  <Button size="sm" variant="outline" className="h-8 text-xs font-semibold text-success border-success/20 hover:bg-success/10" onClick={() => updateProfileField("kyc_status", "approved")}>
