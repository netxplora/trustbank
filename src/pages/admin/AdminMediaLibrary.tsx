  ...
  return (
    <div className="space-y-4 max-w-6xl mx-auto px-1 sm:px-4 py-2 font-sans text-xs">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-2">
        <div>
          <h1 className="text-lg sm:text-xl font-bold font-poppins text-foreground mb-0.5">Media Library</h1>
          <p className="text-xs text-muted-foreground font-sans">Organize your content assets, images, and documents</p>
        </div>
        <div className="flex gap-1.5">
          <Button variant="outline" size="sm" onClick={handleCreateFolder} className="font-bold text-xs h-8 rounded-lg">
            <Folder className="h-3.5 w-3.5 mr-1" /> New Folder
          </Button>
          <Button size="sm" onClick={() => fileInputRef.current?.click()} disabled={uploading} className="font-bold text-xs h-8 rounded-lg">
      ...
      {/* Toolbar */}
      <div className="flex flex-col sm:flex-row gap-4 justify-between bg-card p-3 rounded-3xl border shadow-sm">
        ...
        <div className="flex gap-2 shrink-0">
          <div className="relative">
            ...
            <Input placeholder="Search files..." value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} className="h-11 w-[200px] pl-8 text-base" />
          </div>
          <select value={filterType} onChange={(e: any) => setFilterType(e.target.value)} className="h-11 rounded-md border bg-background px-3 text-base font-semibold">
      ...
      <div className="grid lg:grid-cols-4 gap-6">
        {/* Main Content Area */}
        <SlideUp className={`bg-card border rounded-3xl shadow-sm p-6 min-h-[500px] ${selectedFile ? 'lg:col-span-3' : 'lg:col-span-4'}`}>
          {loading ? (
            viewMode === "grid" ? (
