...
const AdminSettingsPage = () => {
  ...
  return (
    <div className="space-y-4 max-w-6xl mx-auto px-1 sm:px-4 py-2 font-sans text-xs">
      <div>
        <h1 className="text-lg sm:text-xl font-bold font-poppins text-foreground mb-0.5">Brand & Platform Settings</h1>
        <p className="text-xs text-muted-foreground font-sans">Manage global branding, design tokens, and corporate information.</p>
      </div>

      <div className="flex gap-1.5 flex-wrap font-sans">
        {tabs.map(({ key, label, icon: Icon }) => (
          <Button key={key} variant={activeTab === key ? "default" : "outline"} size="sm" onClick={() => setActiveTab(key)} className="font-bold text-xs h-8 rounded-lg">
      ...
      <SlideUp>
      <div className="bg-card rounded-xl border border-border/60 p-3.5 sm:p-4 max-w-4xl shadow-sm font-sans relative">
        <FadeIn key={activeTab}>
        {activeTab === "identity" && (
          <div className="space-y-3.5">
            <h2 className="font-bold font-poppins text-foreground text-xs mb-3 border-b border-border/60 pb-2">Platform Identity</h2>
        ...
        {activeTab === "design" && (
          <div className="space-y-8">
            ...
            <div className="grid grid-cols-1 xl:grid-cols-2 gap-8">
              {/* Light Mode Column */}
              <div className="space-y-6 bg-muted/10 p-5 rounded-3xl border">
              ...
              {/* Dark Mode Column */}
              <div className="space-y-6 bg-slate-900 text-slate-100 p-5 rounded-3xl border border-slate-800">
