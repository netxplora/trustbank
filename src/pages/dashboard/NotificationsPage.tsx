...
const NotificationsPage = () => {
  ...
  if (loading) return (
    <div className="space-y-4 animate-in fade-in duration-300">
      <div className="space-y-1"><div className="h-5 w-28 bg-muted rounded animate-pulse" /><div className="h-3 w-36 bg-muted rounded animate-pulse" /></div>
  ...
  return (
    <div className="space-y-4 max-w-6xl mx-auto px-1 sm:px-4 py-2 font-sans">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-lg sm:text-xl font-bold text-foreground mb-0.5 font-poppins">Notifications</h1>
          <p className="text-xs text-muted-foreground">{unreadCount} unread notification{unreadCount !== 1 ? "s" : ""}</p>
        </div>
        {unreadCount > 0 && <Button variant="outline" size="sm" className="text-xs h-7 rounded-lg" onClick={markAllRead}>Mark all read</Button>}
      </div>
      <FadeIn className="bg-card rounded-xl border border-border/60 divide-y divide-border/40 overflow-hidden shadow-sm">
        {notifications.length === 0 ? (
          <div className="p-8 text-center text-xs font-semibold text-muted-foreground flex flex-col items-center gap-1.5">
          ...
          notifications.map((n) => {
            ...
            return (
              <div key={n.id} className={`px-3.5 py-2.5 flex items-start gap-2.5 cursor-pointer hover:bg-muted/30 transition-colors ${!n.read ? "bg-primary/5" : ""}`} onClick={() => markRead(n.id)}>
                <div className={`h-7 w-7 rounded-full flex items-center justify-center shrink-0 ${colorMap[n.type] || "bg-muted text-muted-foreground"}`}><Icon className="h-3.5 w-3.5" /></div>
