...
const publicAppFile = path.join(rootDir, 'apps', 'public-website', 'src', 'App.tsx');
const bankingAppFile = path.join(rootDir, 'apps', 'banking-app', 'src', 'App.tsx');

const publicContent = `import React, { Suspense, lazy, useEffect } from "react";
...
const bankingContent = `import React, { Suspense, lazy, useEffect } from "react";
import { PageLoader } from "@trustbank/shared-ui/components/ui/PageLoader";
import { QueryClient, QueryClientProvider } from "@tanstack/react-query";
...
fs.writeFileSync(publicAppFile, publicContent);
fs.writeFileSync(bankingAppFile, bankingContent);
console.log("App.tsx rewritten for both apps.");
